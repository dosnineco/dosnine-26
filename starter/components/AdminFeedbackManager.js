...
import { X, MessageCircle, Send, Users, Eye, CheckCircle2, Clock } from 'lucide-react';
...
export default function AdminFeedbackManager() {
  ...
  async function fetchFeedback() {
    ...
    try {
      ...
      console.log('Feedback with responses:', data.feedback?.filter(f => f.agent_response).length);
  ...
  async function handleSendMessage() {
    ...
    try {
      const { data } = await axios.post('/api/admin/feedback', {
        ...
        agentIds: [] // Empty array = send to all agents
      });
  ...
  function viewFeedbackDetail(item) {
    console.log('Viewing feedback detail:', item);
    console.log('Has agent_response?', !!item.agent_response);
    console.log('Agent response:', item.agent_response);
  ...
  return (
    <>
      {/* Feedback Manager Button */}
      <button
        onClick={() => setIsOpen(true)}
        className="relative flex items-center gap-2 px-4 py-2 btn-accent text-white rounded-lg transition-colors"
      >
        <MessageCircle size={20} />
        <span>Agent Feedback</span>
        {unreadResponses > 0 && (
          <span className="absolute -top-2 -right-2 bg-accent text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center animate-pulse">
            {unreadResponses}
          </span>
        )}
      </button>
