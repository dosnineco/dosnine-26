...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_service_requests_client_user_id ON public.service_requests(client_user_id);
CREATE INDEX IF NOT EXISTS idx_service_requests_assigned_agent_id ON public.service_requests(assigned_agent_id);
...
-- Policy: Users can view all requests (simplified for now)
CREATE POLICY "Users can view requests"
  ON public.service_requests
  FOR SELECT
  USING (true);

-- Policy: Users can update requests
